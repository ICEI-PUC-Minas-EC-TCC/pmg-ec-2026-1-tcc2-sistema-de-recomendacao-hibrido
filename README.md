...
## Modelos pré-treinados (atalho para o simulador)

> **Quer rodar o simulador sem treinar nada?** Baixe os modelos já prontos abaixo.

Os modelos treinados para os três scale factors do TCC (SF0.1, SF3 e SF30) estão disponíveis para download. Com eles é possível subir o simulador imediatamente, sem executar o pipeline de extração nem o treinamento.

**Download:** [Modelos Treinados.zip — Google Drive](https://drive.google.com/drive/folders/1IMr5oCgvYNXOx-K6Tijk7ZWMoSZkcGOS?usp=sharing)

> **Substitua o link acima** pelo URL real do Google Drive após o upload do arquivo `Modelos Treinados.zip`.

### Como usar os modelos baixados

1. **Baixe** o arquivo `Modelos Treinados.zip` pelo link acima.

2. **Extraia** o zip. Você obterá uma pasta com a seguinte estrutura interna:
   ```
   Modelos Treinados/
   └── social_network-sf0.1-CsvBasic-LongDateFormatter/
   │   ├── baseline_hibrido_otimizado/
   │   ├── baseline_hibrido_padrao/
   │   ├── ltr_lightgbm_v1_robusto/
   │   └── popularity_baseline/
   ├── social_network-sf3-CsvBasic-LongDateFormatter/
   │   └── ...
   └── social_network-sf30-CsvBasic-LongDateFormatter/
       └── ...
   ```

3. **Mova o conteúdo** da pasta extraída para `treinamento/modelos/` dentro do projeto:
   - **Windows (Explorer):** Abra a pasta `Modelos Treinados`, selecione tudo (`Ctrl+A`), copie (`Ctrl+C`) e cole dentro de `treinamento/modelos/`.
   - **Windows (terminal):**
     ```powershell
     robocopy "Modelos Treinados" "treinamento\modelos" /E
     ```
   - **Linux/macOS:**
     ```bash
     cp -r "Modelos Treinados/." treinamento/modelos/
     ```

4. **Suba o simulador** a partir da raiz do projeto:
   ```bash
   python -m simulador.api
   ```
   Acesse `http://localhost:8001` no navegador. Os modelos aparecerão automaticamente no dropdown.

> Os artefatos de cada modelo ficam em `treinamento/modelos/<dataset_key>/<model_id>/` e incluem `vectorizer.pkl`, `post_matrix.npy`, `posts_cache.parquet`, `metadata.json` e, para modelos LTR, `ltr_model.txt`.
